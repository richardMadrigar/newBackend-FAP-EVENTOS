...
export const updateUser = async (req: Request, res: Response): Promise<Response> => {
  ...
  const {
    nome_completo, email, celular, whats, cpf, rg, pix, data_nascimento, nit_pis,
    nome_da_mae, banco, agencia, conta, cep, numero_da_rua,
  } = req.body;

  const SQL = `
      UPDATE usuarios SET
      nome_completo = $1, email = $2,  cpf = $3,  rg = $4, whats = $5, celular = $6, 
      data_nascimento = $7, nit_pis = $8, nome_da_mae = $9, banco = $10,
      agencia = $11, conta = $12, cep = $13, pix = $14, numero_da_rua = $15  

      WHERE id_usuario = $16 `;

  const values = [
    nome_completo, email, cpf, rg, whats, celular, data_nascimento, nit_pis,
    nome_da_mae, banco, agencia, conta, cep, pix, numero_da_rua, id_usuario,
  ];
